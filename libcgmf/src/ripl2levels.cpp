...
static Level **allLevels; // all discrete levels from RIPL database
static int *zaidIndex;
static int *numberLevels;
static int NUM_NUCLEI = 0;
const int MAX_NUCLEI = 3500;
const int MAX_GAMMA_BRANCH = 61;
		...
		for(int i=0 ; i<nol ; i++){ // loop over discrete levels
			...
			for(int j=0 ; j<ng ; j++){
				if(j==0){
					...
					allLevels[c][i].fstate.push_back(nf[j]);
					allLevels[c][i].branch.push_back(pe[j]);
					allLevels[c][i].gratio.push_back(1.0/(1.0 + ic[j])); // Ig/(Ig + Ie) = 1/(1+ICC)
				}
			}
      ...
      if(found && i<nlevel){
        ...
        for(int j=0 ; j<ng ; j++){
          if(j==0){
            ...
            lev[i].fstate.push_back(nf[j]);
            lev[i].branch.push_back(pe[j]);
            lev[i].gratio.push_back(1.0/(1.0 + ic[j])); // Ig/(Ig + Ie) = 1/(1+ICC)
          }
        }
      }
    }
